...
| POST | `/api/spend` | Atomic, idempotent debit for shop purchases (proposed: enforces the per-player weekly cap for `grant_item` consumables) |
...
| GET | `/api/state/{steam64}` | One-shot balance + leaderboard + totals snapshot |
