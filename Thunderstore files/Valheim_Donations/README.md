...
- **A configurable shop** (`valcoin_shop.yaml`) sells:
  ...
  - **Familiars** (`armor_vfx`) — a miniature flying creature (Bat, Ghost, Deathsquito, Drake, Wraith, Volture, Gjall, Fallen Valkyrie) hovers at your shoulder, bound to your equipped helmet. Grants feather fall and a small flat attack bonus. Other players see it too. **Move yours** by editing `BepInEx/config/valcoin_familiars.yaml` — one block per familiar, left/right, height, front/back — and it repositions within a second, in-game, no re-equip. Your view only.
